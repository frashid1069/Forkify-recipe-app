...
}};class $ extends b{_parentElement=document.querySelector(".recipe");_errorMessage="We could not find that recipe. Please try another one!";_message="";addHandlerRender(e){["hashchange","load"].forEach(t=>window.addEventListener(t,e))}addHandlerUpdateServings(e){this._parentElement.addEventListener("click",function(t){let r=t.target.closest(".btn--update-servings");if(!r)return;let{updateTo:n}=r.dataset;+n>0&&e(+n)})}addHandlerAddBookmark(e){this._parentElement.addEventListener("click",function(t){let r=t.target.closest(".btn--bookmark");r&&e()})}_generateMarkup(){return`
...
await m(e),console.log(l.recipe),E.render(l.recipe),T.renderMessage(),A.render(l.bookmarks),// Change ID in URL
window.history.pushState(null,"",`#${l.recipe.id}`),// Close form window
setTimeout(function(){T.toggleWindow()},2500)}catch(e){console.error("\uD83D\uDCA5",e),T.renderError(e.message)}};A.addHandlerRender(function(){A.render(l.bookmarks)}),E.addHandlerRender(F),E.addHandlerUpdateServings(function(e){// Update the recipe servings (in state)
g(e),E.update(l.recipe)}),E.addHandlerAddBookmark(function(){l.recipe.bookmarked?v(l.recipe.id):_(l.recipe),E.update(l.recipe),A.render(l.bookmarks)}),x.addHandlerSearch(N),P.addHandlerClick(function(e){M.render(p(e)),P.render(l.search)}),T.addHandlerUpload(R);//# sourceMappingURL=index.fd5491c6.js.map

//# sourceMappingURL=index.fd5491c6.js.map
